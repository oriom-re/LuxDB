...
class LuxSafeManager:
    ...
    def _log_access_attempt(self, profile_id: Optional[str], identifier: str, 
                           resource: str, granted: bool, required_level: int, 
                           actual_level: int, resonance: float, context: Optional[Dict] = None,
                           auth_method: str = "unknown"):
        """Zaloguj próbę dostępu do zasobów"""

        # Sprawdź czy identifier to soul_name czy device_fingerprint
        soul_name = None
        device_fingerprint = identifier

        if not identifier.startswith("Ω-"):
            soul_name = identifier
            device_fingerprint = f"unknown_device_{hashlib.sha256(identifier.encode()).hexdigest()[:16]}"
