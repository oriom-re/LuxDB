...
class SecureCodeFlow:
    ...
    def generate_secure_function(self, intention: IntentionBeing, 
        ...
        try:
            ...
            # 3.5 Walidacja przez Federację (jeśli dostępna)
            federation_validation = self._validate_with_federation(generated_code, context)
            if federation_validation and not federation_validation.get('code_approved', True):
                return {
                    'status': 'federation_rejected',
                    'reason': 'Kod nie spełnia standardów Federacji',
                    'federation_feedback': federation_validation
                }
...
'''
    ...
    def _validate_with_federation(self, code: str, context: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Waliduje kod przez Federację jeśli jest dostępna
        """
        try:
            # Sprawdź czy Federation Flow jest dostępny
            federation_flow = self.engine.flows.get('federation')
            if not federation_flow:
                return None
            
            # Przeprowadź walidację przez Federację
            validation_result = federation_flow.validate_generated_code(code, context)
            
            self.engine.logger.info(f"🌐 Kod zwalidowany przez Federację: {validation_result.get('code_approved', False)}")
            
            return validation_result
            
        except Exception as e:
            self.engine.logger.warning(f"⚠️ Błąd walidacji przez Federację: {e}")
            return None
    
    def prepare_code_for_github_via_federation(self, func_id: str, target_repo: str) -> Dict[str, Any]:
        """
        Przygotowuje kod do wysłania na GitHub przez kontrolę Federacji
        """
        try:
            # Znajdź funkcję w warstwach wymiarów
            func = None
            ...
            federation_flow = self.engine.flows.get('federation')
            if not federation_flow:
                return {'status': 'federation_unavailable', 'error': 'Federation Flow nie jest dostępny'}
            
            # Przeprowadź inspekcję przez Federację
            github_inspection = federation_flow.inspect_sandbox_code_before_github(func.code, target_repo)
            
            if github_inspection.get('ready_for_github', False):
                # Przygotuj finalny kod z metadanymi
                final_code = self._add_metadata_for_github(func, target_repo)
                ...
                return {
                    ...
                    'code': final_code,
                    'federation_inspection': github_inspection,
                    'metadata': {
                        'function_name': func.name,
                        'purpose': func.purpose,
                        'created_by': func.created_by,
                        'execution_count': func.execution_count,
                        'dimension_layer': func.dimension_layer
                    }
                }
            else:
                return {
                    'status': 'github_rejected',
                    'function_id': func_id,
                    'reason': 'Kod nie spełnia wymagań GitHub',
                    'federation_feedback': github_inspection
                }
                
        except Exception as e:
            self.engine.logger.error(f"❌ Błąd przygotowania kodu dla GitHub: {e}")
            return {
                'status': 'error',
                'error': str(e)
            }
    
    def _add_metadata_for_github(self, func: GeneratedFunction, target_repo: str) -> str:
        """
        Dodaje metadane do kodu przed wysłaniem na GitHub
        """
        metadata_header = f'''"""
Generated by Astra SecureCodeFlow - Federation Approved
Function: {func.name}
Purpose: {func.purpose}
Created: {func.created_by}
Execution Count: {func.execution_count}
Dimension Layer: {func.dimension_layer}
Target Repository: {target_repo}
Security Level: Validated
Federation Compliance: Approved

This code was generated in a secure sandbox environment
and passed both security and Federation quality controls.
"""

'''
        
        return metadata_header + func.code
